...
class CmsAuthor(Base):
    ...
    posts: Mapped[list["CmsPost"]] = relationship(
        "CmsPost", back_populates="author", lazy="select"
    )
...
class CmsCategory(Base):
    ...
    posts: Mapped[list["CmsPost"]] = relationship(
        "CmsPost", back_populates="category", lazy="select"
    )
...
class CmsPost(Base):
    ...
    content_type: Mapped[str] = mapped_column(
        String(64), nullable=False, default="blog_article"
    )
    ...
    author: Mapped[Optional["CmsAuthor"]] = relationship(
        "CmsAuthor", back_populates="posts", lazy="joined"
    )
    category: Mapped[Optional["CmsCategory"]] = relationship(
        "CmsCategory", back_populates="posts", lazy="joined"
    )
...
class CmsSocialEmbed(Base):
    ...
    related_post_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("cms_posts.id", ondelete="SET NULL"),
        nullable=True,
    )
