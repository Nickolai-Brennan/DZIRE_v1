    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-primary text-sm uppercase tracking-widest mb-1">My Collection</p>
        ...
        {playlists.length === 0 ? (
          <div className="text-center py-20 text-textMuted">
            ...
            <p className="mb-6">Save positions to create your first playlist.</p>
            <Link to="/positions">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <Link to={`/playlists/${playlist.id}`}>
                      <h3 className="font-bold text-textPrimary hover:text-primary transition-colors">{playlist.name}</h3>
                    </Link>
                    <p className="text-sm text-textMuted">{playlist.items.length} item{playlist.items.length !== 1 ? 's' : ''}</p>
                  </div>
                ...
                {playlist.items.length > 0 ? (
                  <div className="flex gap-1 flex-wrap">
                    {playlist.items.slice(0, 3).map(item => (
                      <Badge key={item.id} variant="category">{item.type}</Badge>
                    ))}
                    {playlist.items.length > 3 && (
                      <Badge variant="category">+{playlist.items.length - 3} more</Badge>
                    )}
                  </div>
                ) : (
                  <p className="text-xs text-textMuted">Empty — add positions to get started</p>
                )}

                <Link to={`/playlists/${playlist.id}`} className="block mt-4">
                  <Button variant="secondary" className="w-full text-sm">Open Playlist</Button>
                </Link>
              </Card>
            ))}
          </div>
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create New Playlist">
