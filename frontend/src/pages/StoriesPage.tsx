...
const CATEGORY_FILTERS = ['All', 'Romance', 'Slow Burn', 'Adventure', 'Forbidden'];
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="bg-gradient-to-b from-surfaceAlt to-background py-16 px-4 sm:px-6 text-center mb-12">
        <p className="text-primary text-sm uppercase tracking-widest mb-3">DZIRE Stories</p>
        <h1 className="text-4xl md:text-6xl font-black text-textPrimary mb-4">Fantasy & Desire</h1>
        <p className="text-textMuted text-lg max-w-xl mx-auto">
          Sensual fiction and intimate stories celebrating connection, desire, and discovery.
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Filters */}
        <div className="mb-8">
          <FilterChips filters={CATEGORY_FILTERS} active={activeFilter} onChange={setActiveFilter} />
        </div>
          ...
          {filtered.map(story => (
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-textMuted text-sm leading-relaxed mb-4 line-clamp-2">{story.excerpt}</p>
                ...
                <div className="flex flex-wrap gap-1 mt-3">
                  {story.tags.slice(0, 3).map(tag => (
                    <span key={tag} className="text-xs text-textMuted">#{tag}</span>
                  ))}
                </div>
              </div>
            </Link>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-textMuted">
            <p className="text-2xl font-bold mb-2">No stories in this category</p>
