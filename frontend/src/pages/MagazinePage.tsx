...
export const MagazinePage: React.FC = () => {
  ...
  const issueArticles = mockArticles.filter((a) =>
    latestIssue.articles.includes(a.id),
  );
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 mb-12">
            {mockMagazine.map((issue) => (
              <button
                key={issue.id}
                onClick={() =>
                  track("magazine_issue_click", { issueId: issue.id })
                }
              ...
              >
                ...
                <p className="text-xs text-textMuted">
                  {issue.coverDate} · Issue #{issue.issueNumber}
                </p>
              </button>
            ))}
          </div>
        ...
        <section className="mb-16">
          ...
          {issueArticles[0] && (
            <Link
              ...
              onClick={() =>
                track("article_click", { articleId: issueArticles[0].id })
              }
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="relative aspect-video md:aspect-auto overflow-hidden">
                  <img
                    src={issueArticles[0].featuredImage}
                    alt={issueArticles[0].title}
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                  />
                </div>
                <div className="p-8 flex flex-col justify-center">
                  <Badge variant="category" className="mb-3 self-start">
                    {issueArticles[0].category}
                  </Badge>
                  ...
                  <p className="text-textMuted leading-relaxed mb-4">
                    {issueArticles[0].excerpt}
                  </p>
                  <div className="flex items-center gap-4 text-sm text-textMuted">
                    <span className="flex items-center gap-1">
                      <User className="w-3 h-3" />
                      {issueArticles[0].author}
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3" />
                      {issueArticles[0].readingTime} min read
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {issueArticles.slice(1).map((article) => (
              <Link
                ...
                onClick={() =>
                  track("article_click", { articleId: article.id })
                }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {mockArticles.map((article) => (
              <Link
                ...
                to={`/search?q=${encodeURIComponent(article.title)}`}
                onClick={() =>
                  track("article_list_click", { articleId: article.id })
                }
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <h4 className="font-bold text-textPrimary text-sm group-hover:text-primary transition-colors line-clamp-2">
                    {article.title}
                  </h4>
