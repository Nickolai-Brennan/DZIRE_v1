...
export const VIPSubscribePage: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/api/vip/plans')
      .then((r) => (r.ok ? r.json() : []))
      .then((data: VipPlan[]) => setPlans(data.filter((p) => p.status === 'active')))
      .catch(() => setPlans([]))
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold text-textPrimary mb-4">VIP Membership</h1>
        <p className="text-textMuted text-lg max-w-xl mx-auto">
          Get exclusive access to VIP-only content, early releases, and premium features.
        </p>
      </div>
      ...
      {!loading && plans.length === 0 && (
        <p className="text-center text-textMuted italic">No VIP plans available right now.</p>
      )}
        ...
        {plans.map((plan) => (
          ...
          >
            ...
            <div className="text-3xl font-bold text-primary mb-1">
              ${plan.price}
              <span className="text-sm text-textMuted font-normal">/{plan.billing_interval}</span>
            </div>
            {plan.description && (
