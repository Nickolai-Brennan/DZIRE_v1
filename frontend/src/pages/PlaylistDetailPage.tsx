...
export const PlaylistDetailPage: React.FC = () => {
  ...
  const handleStartMode = () => {
    ...
    if (playlist.items[0]) {
      window.location.href = getItemHref(
        playlist.items[0].type,
        playlist.items[0].slug,
      );
    }
  };
        ...
        {playlist.items.length === 0 ? (
          ...
          <div className="space-y-3">
            {playlist.items.map((item, index) => (
              ...
              >
                <span className="text-textMuted text-sm font-mono w-6 text-center">
                  {index + 1}
                </span>
                ...
                <div className="flex gap-1 shrink-0">
                  {index > 0 && (
                    <button
                      onClick={() =>
                        reorderPlaylist(playlist.id, index, index - 1)
                      }
