...
const FAQItem: React.FC<{ question: string; answer: string }> = ({
  ...
  return (
    <div className="border-b border-white/8">
      ...
      {open && (
        <p className="pb-4 text-textMuted text-sm leading-relaxed">{answer}</p>
      )}
    </div>
  );
};
...
export const ReviewDetailPage: React.FC = () => {
  ...
  const faqs = [
    {
      question: "Is this product beginner-friendly?",
      answer: `With a beginners score of ${review.beginnerFriendly}/10, ${review.title} is ${review.beginnerFriendly >= 8 ? "highly recommended for beginners" : "better suited for intermediate users"}.`,
    },
    {
      question: "How is the build quality?",
      answer: `Build quality score: ${review.buildQuality}/10. The materials and construction ${review.buildQuality >= 8 ? "are excellent and built to last" : "are decent for the price point"}.`,
    },
    {
      question: "Is it worth the price?",
      answer: `Value score: ${review.valueScore}/10. At $${review.price} (${review.priceRange}), it ${review.valueScore >= 8 ? "offers outstanding value" : "provides fair value for the quality"}.`,
    },
  ];
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mb-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          ...
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-black text-textPrimary mb-4">
              {review.title}
            </h1>
            ...
            <a
              ...
              onClick={() =>
                track("review_offer_click", { reviewId: review.id })
              }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
              ...
              <section className="bg-surface rounded-2xl p-6 border border-white/8">
                ...
                <div className="flex flex-wrap gap-2">
                  {review.awards.map((award) => (
                    <Badge key={award} variant="trophy">
                      {award}
                    </Badge>
                  ))}
                </div>
            ...
            <section className="bg-surfaceAlt rounded-2xl p-4 border border-white/8">
              <p className="text-xs text-textMuted italic">
                Disclosure: Some links on this page are affiliate links. DZIRE
                may earn a commission if you purchase through these links at no
                extra cost to you. All reviews are based on independent
          ...
          <div className="space-y-6">
            ...
            <div className="bg-surface rounded-2xl p-5 border border-primary/20 text-center">
              ...
              <p className="text-xl font-black text-textPrimary mb-1">
                ${review.price}
              </p>
            ...
            <div className="bg-surface rounded-2xl p-5 border border-white/8">
              <h3 className="text-sm font-bold text-textPrimary uppercase tracking-wider mb-3">
                Use With These Positions
              </h3>
                ...
                {useWithPositions.map((pos) => (
                  <div key={pos.id} className="flex items-center gap-2">
                    <Link
                      ...
                      className="flex-1 text-xs text-textMuted hover:text-primary transition-colors truncate"
                    >
                      {pos.title}
                    </Link>
