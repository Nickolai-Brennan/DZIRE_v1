...
export const DictionaryPage: React.FC = () => {
  ...
  const categories = [
    "All",
    ...Array.from(new Set(mockDictionary.map((t) => t.category))),
  ];
  ...
  const filtered = useMemo(() => {
    return mockDictionary.filter((t) => {
      ...
      const matchLetter =
        activeLetter === "All" || t.term.toUpperCase().startsWith(activeLetter);
      const matchCat =
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-gradient-to-b from-surfaceAlt to-background py-16 px-4 sm:px-6 text-center mb-12">
        ...
        <h1 className="text-4xl md:text-6xl font-black text-textPrimary mb-4">
          Sex & Intimacy Glossary
        </h1>
        <p className="text-textMuted text-lg max-w-xl mx-auto mb-8">
          Clear, judgment-free definitions for the language of pleasure and
          connection.
        </p>
          ...
          <h2 className="text-2xl font-bold text-textPrimary mb-6">
            {searchQuery
              ...
              : activeLetter !== "All"
                ...
                : "All Terms"}
            <span className="text-base font-normal text-textMuted ml-3">
              ({filtered.length})
            </span>
          </h2>
          {filtered.length > 0 ? (
