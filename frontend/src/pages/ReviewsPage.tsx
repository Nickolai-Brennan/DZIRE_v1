...
export const ReviewsPage: React.FC = () => {
  ...
  const trophyWinners = TROPHY_AWARDS.map(award => ({
    award: award.label,
    review: mockReviews.find(r => r.awards.includes(award.id)) || mockReviews[0],
  }));
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="bg-gradient-to-b from-surfaceAlt to-background py-16 px-4 sm:px-6 text-center mb-12">
        <p className="text-gold text-sm uppercase tracking-widest mb-3">DZIRE Reviews</p>
        <h1 className="text-4xl md:text-6xl font-black text-textPrimary mb-4">Product Reviews</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="mb-16">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-3xl font-black text-textPrimary">DZIRE Trophy Hall of Fame</h2>
          </div>
        ...
        <div className="flex flex-wrap gap-4 justify-center mb-12">
          {['Editorial Independence', 'Expert-Tested', 'Community Rated', 'No Pay-to-Win Reviews', 'Updated Regularly'].map((badge) => (
            <span key={badge} className="px-4 py-2 bg-surface border border-white/10 rounded-full text-sm text-textMuted">
              ✓ {badge}
            </span>
          ))}
        </div>
        ...
        {/* Category Filters */}
        <div className="mb-8">
          <FilterChips filters={CATEGORY_FILTERS} active={activeCategory} onChange={setActiveCategory} />
        </div>

        {/* Review Grid */}
        <section className="mb-16">
          <h2 className="text-2xl font-bold text-textPrimary mb-6">
            {activeCategory === 'All' ? 'All Reviews' : `${activeCategory} Reviews`}
          </h2>
