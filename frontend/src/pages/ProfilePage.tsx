    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4">
        ...
        <p className="text-textMuted text-center max-w-sm">
          Create an account or log in to save playlists, track favourites, and
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            {
              label: "Saved Items",
              value: playlists.reduce((sum, p) => sum + p.items.length, 0),
              icon: User,
            },
        ...
        <div className="mb-8">
          ...
          {recentPlaylists.length > 0 ? (
              ...
              {recentPlaylists.map((playlist) => (
                <Link key={playlist.id} to={`/playlists/${playlist.id}`}>
                  <Card hover className="p-5">
                    <p className="font-bold text-textPrimary mb-1">
                      {playlist.name}
                    </p>
            ...
            <p className="text-textMuted text-sm">
              No playlists yet. Save positions to get started.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {recentFavorites.length > 0 ? (
              ...
              {recentFavorites.map((fav) => (
                <Link key={fav.id} to={`/${fav.type}s/${fav.slug}`}>
                  <div className="bg-surface rounded-xl border border-white/10 p-3 text-center hover:border-primary/30 transition-colors">
                    ...
                    <p className="text-xs text-textPrimary font-medium line-clamp-2">
                      {fav.title}
                    </p>
                  </div>
                </Link>
            ...
            <p className="text-textMuted text-sm">
              No favourites yet. Click the heart on any content to save it here.
            </p>
          )}
        </div>
      </div>
