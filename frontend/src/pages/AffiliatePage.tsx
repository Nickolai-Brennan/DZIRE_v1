...
export const AffiliatePage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold text-textPrimary mb-4">Affiliate Deals</h1>
      <p className="text-textMuted mb-8">
        Exclusive deals and recommendations from DZIRE. Some links are affiliate links — we may
        earn a commission at no cost to you.
      </p>
      {loading && <p className="text-textMuted">Loading…</p>}
      {!loading && links.length === 0 && (
        <p className="text-textMuted italic">Deals and recommendations coming soon.</p>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {links.map((link) => (
          ...
          >
            <h2 className="text-lg font-semibold text-textPrimary mb-2">{link.product_name}</h2>
            {link.coupon_code && (
