...
export const DictionaryTermPage: React.FC = () => {
  ...
  const relatedTerms = mockDictionary.filter((t) =>
    term?.relatedTerms.includes(t.id),
  );
  const relatedPositions = mockPositions.filter((p) =>
    term?.relatedPositions.includes(p.id),
  );
  ...
  if (!term) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <Link to="/dictionary" className="text-primary hover:underline">
          Back to Dictionary
        </Link>
      </div>
    );
  ...
  const faqs = [
    { question: `What does "${term.term}" mean?`, answer: term.shortDef },
    {
      ...
      answer: `"${term.term}" is pronounced /${term.pronunciation}/.`,
    },
    {
      question: `What category does "${term.term}" belong to?`,
      answer: `"${term.term}" falls under the ${term.category} category.`,
    },
  ];
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-gradient-to-b from-surfaceAlt to-background pt-10 pb-14 px-4 sm:px-6 mb-12">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-black text-textPrimary mb-3">
            {term.term}
          </h1>
          {term.pronunciation && (
            <p className="text-xl text-textMuted italic mb-4">
              / {term.pronunciation} /
            </p>
          )}
