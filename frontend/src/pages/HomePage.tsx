...
export const HomePage: React.FC = () => {
  ...
  const trendingPositions = mockPositions.filter(p => p.isFeatured).slice(0, 4);
  ...
  return (
    <div className="w-full">
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-3xl font-bold text-textPrimary">Featured DZIRE Dolls</h2>
            <Link to="/dzire-dolls" className="text-primary hover:text-accent transition-colors flex items-center gap-2">
            ...
            {featuredDolls.map((doll) => (
              <Link key={doll.id} to={`/dzire-dolls/${doll.slug}`}>
                <Card hover className="overflow-hidden">
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-bold text-textPrimary mb-2">{doll.name}</h3>
                    <p className="text-sm text-textMuted mb-4">{doll.tagline}</p>
      ...
      <section className="py-20 bg-surfaceAlt/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-3xl font-bold text-textPrimary">Trending Positions</h2>
            <Link to="/positions" className="text-primary hover:text-accent transition-colors flex items-center gap-2">
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="text-textPrimary font-bold">{position.comfort}/10</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-textMuted">Intimacy</span>
                        <span className="text-textPrimary font-bold">{position.intimacy}/10</span>
                      </div>
                    </div>
                  </div>
                </Card>
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link to="/reviews" className="text-primary hover:text-accent transition-colors flex items-center gap-2">
            ...
            {trophyReviews.map((review) => (
              <Link key={review.id} to={`/reviews/${review.slug}`}>
                <Card hover className="overflow-hidden border-gold/20">
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-bold text-textPrimary mb-2">{review.title}</h3>
                    <p className="text-sm text-textMuted mb-4">{review.excerpt}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-2xl font-bold text-gold">{review.overallScore}</span>
                      <span className="text-sm text-textMuted">Overall Score</span>
                    </div>
                  </div>
                </Card>
              </Link>
      ...
      <section className="py-20 bg-surfaceAlt/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-3xl font-bold text-textPrimary">Sex Dictionary</h2>
            <Link to="/dictionary" className="text-primary hover:text-accent transition-colors flex items-center gap-2">
            ...
            {featuredTerms.map((term) => (
              <Link key={term.id} to={`/dictionary/${term.slug}`}>
                <Card hover className="p-6">
                  ...
                  <h3 className="text-xl font-bold text-textPrimary mb-2">{term.term}</h3>
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-3xl font-bold text-textPrimary">Fantasy Stories</h2>
            <Link to="/stories" className="text-primary hover:text-accent transition-colors flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {latestStories.map((story) => (
              <Link key={story.id} to={`/stories/${story.slug}`}>
                <Card hover className="overflow-hidden">
                  <img src={story.image} alt={story.title} className="w-full aspect-video object-cover" />
                  <div className="p-6">
                    <Badge variant="category" className="mb-3">{story.category}</Badge>
                    <h3 className="text-lg font-bold text-textPrimary mb-2">{story.title}</h3>
                    <p className="text-sm text-textMuted mb-4">{story.excerpt}</p>
      ...
      <section className="py-20 bg-surfaceAlt">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold text-textPrimary mb-4">Stay in the Loop</h2>
          <p className="text-lg text-textMuted mb-10 max-w-2xl mx-auto">
            Get weekly guides, reviews, and stories delivered to your inbox. No spam, just desire.
          </p>
          <NewsletterForm />
        </div>
      </section>
      ...
      <section className="py-20 bg-gradient-to-br from-primary/20 to-accent/20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold text-textPrimary mb-4">Unlock DZIRE VIP Access</h2>
