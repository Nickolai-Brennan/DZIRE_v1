    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      <div className="glass-card overflow-hidden mb-8">
        <div className="relative">
          <img src={doll.image} alt={doll.name} className="w-full h-72 sm:h-96 object-cover"/>
          ...
          {doll.isFeatured && <div className="absolute top-4 right-4 bg-rose-500 text-white text-sm font-bold px-3 py-1 rounded-full">Featured</div>}
        </div>
        <div className="p-8">
          ...
          {doll.promoUrl && (
            <a href={doll.promoUrl} target="_blank" rel="noopener noreferrer"
              onClick={()=>track(EVENTS.DOLL_PROMO_CLICK,{slug:doll.slug})}
              className="inline-flex items-center gap-2 px-6 py-3 bg-rose-500 hover:bg-rose-400 text-white font-semibold rounded-lg transition-colors mb-8">
              <ExternalLink className="w-4 h-4"/>View Full Profile
            </a>
          )}
          <h2 className="text-xl font-black text-white mb-3">About</h2>
          <p className="text-white/60 leading-relaxed mb-8">{doll.bio}</p>
          <h2 className="text-xl font-black text-white mb-3">Connect</h2>
          <div className="flex flex-wrap gap-3 mb-8">
            {doll.platforms.map(p=>(
              <a key={p.name} href={p.url} target="_blank" rel="noopener noreferrer"
                onClick={()=>track(EVENTS.AFFILIATE_CLICK,{platform:p.name,slug:doll.slug})}
