...
export function ReviewDetailPage({ slug }: Props) {
  ...
  const scores = [
    { label:'Ease of Use', val: review.easeOfUseScore },{ label:'Build Quality', val: review.buildQualityScore },
    { label:'Beginner Friendly', val: review.beginnerFriendlyScore },{ label:'Value', val: review.valueScore },
  ];
  return (
    <PublicLayout>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        {review.awardBadge && (
          <div className="flex items-center gap-2 mb-4 text-yellow-400 font-bold text-sm"><Trophy className="w-4 h-4"/>{review.awardBadge}</div>
        )}
        <span className="text-xs text-rose-500 uppercase tracking-widest font-semibold">{review.category}</span>
        <h1 className="text-4xl sm:text-5xl font-black text-white mt-2 mb-4">{review.productName}</h1>
        <img src={review.imagePlaceholder} alt={review.productName} className="w-full rounded-xl mb-8 aspect-video object-cover"/>
        ...
        <div className="glass-card p-5 mb-8">
          ...
          <div className="flex flex-wrap gap-2">{review.bestFor.map(b=><span key={b} className="px-3 py-1.5 bg-white/5 border border-white/10 text-white/60 rounded-full text-sm">{b}</span>)}</div>
        </div>
        {review.affiliateUrl && (
