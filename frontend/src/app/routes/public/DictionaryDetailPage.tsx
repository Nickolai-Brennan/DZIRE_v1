...
export function DictionaryDetailPage({ slug }: Props) {
  ...
  const term = mockDictionaryTerms.find(t => t.slug === slug) || mockDictionaryTerms[0];
  const related = mockDictionaryTerms.filter(t => term.relatedTerms.includes(t.id));
  return (
    <PublicLayout>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="glass-card p-6 mb-8">
          <p className="text-lg text-white/70 leading-relaxed mb-4">{term.shortDefinition}</p>
          <p className="text-white/50 leading-relaxed">{term.expandedDefinition}</p>
        </div>
        <div className="flex flex-wrap gap-2 mb-10">
          {term.tags.map(t=><span key={t} className="px-3 py-1.5 bg-white/5 border border-white/10 text-white/60 rounded-full text-sm">{t}</span>)}
        </div>
        {related.length > 0 && (
          <div>
