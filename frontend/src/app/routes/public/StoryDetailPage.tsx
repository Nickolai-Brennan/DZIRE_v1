    ...
    <PublicLayout>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="flex items-center gap-4 mb-8 text-sm text-white/40">
          ...
          <div className="flex items-center gap-1 text-yellow-400"><Star className="w-3 h-3"/>{story.rating}</div>
          <div className="flex items-center gap-1"><BookOpen className="w-3 h-3"/>{story.popularity.toLocaleString()} reads</div>
        </div>
        <img src={story.coverPlaceholder} alt={story.title} className="w-full rounded-xl mb-10 aspect-video object-cover"/>
