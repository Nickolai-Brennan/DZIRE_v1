...
export function MagazinePage() {
  ...
  const [featured, ...issues] = mockMagazine;
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        {featured && (
          <div className="glass-card border border-rose-500/20 overflow-hidden mb-16 flex flex-col sm:flex-row">
            <img src={featured.coverImage} alt={featured.title} className="w-full sm:w-64 object-cover"/>
            <div className="p-8 flex flex-col justify-center">
              <p className="text-rose-500 text-xs font-bold uppercase tracking-widest mb-2">Issue {featured.issueNumber} · {featured.coverDate}</p>
              <h2 className="text-3xl font-black text-white mb-6">{featured.title}</h2>
              <ul className="space-y-2 mb-8">{featured.articles.map(t=><li key={t} className="flex items-center gap-2 text-sm text-white/60"><BookOpen className="w-3 h-3 text-rose-500"/>{t}</li>)}</ul>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {issues.map(issue=>(
            <div key={issue.id} className="glass-card overflow-hidden flex flex-col sm:flex-row">
              <img src={issue.coverImage} alt={issue.title} className="w-full sm:w-32 object-cover aspect-[3/4] sm:aspect-auto"/>
              <div className="p-4">
                <h3 className="font-bold text-white mb-1">{issue.title}</h3>
                <p className="text-xs text-white/40 mb-3">Issue {issue.issueNumber} · {issue.coverDate}</p>
