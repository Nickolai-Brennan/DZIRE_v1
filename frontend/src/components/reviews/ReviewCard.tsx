...
export const ReviewCard: React.FC<ReviewCardProps> = ({
  ...
  return (
    <div
      className={`group bg-surface rounded-2xl overflow-hidden border transition-all duration-300 hover:-translate-y-1 flex flex-col ${review.isTrophy ? "border-gold/20 hover:border-gold/40" : "border-white/8 hover:border-primary/30"}`}
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        <Badge variant="category" className="mb-2 self-start">
          {review.category}
        </Badge>
        <p className="text-xs text-textMuted mb-1">{review.brand}</p>
        <h3
          className={`font-bold text-textPrimary mb-2 ${featured ? "text-xl" : "text-lg"}`}
        >
          {review.title}
        </h3>
