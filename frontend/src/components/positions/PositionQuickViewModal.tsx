...
const compatibilityQuestions = [
  ...
  },
  {
    id: "partnerSetup",
    label: "Partner Setup",
    options: ["Solo", "Partner", "Multiple"],
  },
];

export const PositionQuickViewModal: React.FC<PositionQuickViewModalProps> = ({
  position,
  onClose,
}) => {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/10 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-textMuted hover:text-textPrimary transition-colors p-1 rounded-lg hover:bg-surfaceAlt"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="flex flex-col sm:flex-row">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {position.relatedPositions.map((id) => (
                      <span
                        key={id}
                        className="text-xs text-primary border border-primary/30 px-2 py-1 rounded"
                      >
                        {id}
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {position.relatedTerms.map((id) => (
                      <span
                        key={id}
                        className="text-xs text-textMuted border border-white/10 px-2 py-1 rounded"
                      >
                        {id}
                      </span>
                    ))}
            ...
            <div className="flex-1 p-5">
              ...
              <div className="mb-6 space-y-3">
                ...
                <ScoreBar
                  label="Comfort"
                  value={position.comfort}
                  color="primary"
                />
                <ScoreBar
                  label="Energy"
                  value={position.energy}
                  color="primary"
                />
                <ScoreBar
                  label="Difficulty"
              ...
              <div className="mb-6">
                ...
                <p className="text-xs text-textMuted mt-3 italic">
                  Your selections are private and used only to refine
                  suggestions.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link
                  ...
                  onClick={() => {
                    track("position_full_guide_click", {
                ...
                <button
                  onClick={() => {
                    track("save_position_click", { positionId: position.id });
                  }}
