    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" role="dialog" aria-modal="true" aria-label={`Quick view: ${position.title}`}>
      ...
      <div className="relative w-full sm:max-w-4xl max-h-[90vh] overflow-y-auto glass-card rounded-t-2xl sm:rounded-2xl p-6 z-10">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          <div>
            <img src={position.image} alt={position.title} className="w-full rounded-xl mb-4"/>
          ...
          <div>
            <p className="text-white/60 text-sm mb-5">{position.description}</p>
            ...
            <div className="flex flex-col gap-2">
              <Link to={`/positions/${position.slug}`} onClick={onClose} className="text-center py-2.5 bg-rose-500 hover:bg-rose-400 text-white font-semibold rounded-lg transition-colors">Open Full Guide</Link>
