...
export function DictionaryTermCard({ term }: Props) {
  return (
    <div className="glass-card p-5">
      ...
      <p className="text-sm text-white/60 mb-4">{term.shortDef}</p>
      <div className="flex flex-wrap gap-1 mb-4">
        {term.relatedTerms.map(t=><span key={t} className="text-xs px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/50">{t}</span>)}
      </div>
      <Link to={`/dictionary/${term.slug}`} className="text-sm text-rose-400 hover:text-rose-300 font-semibold transition-colors">Read More →</Link>
    </div>
  );
}
