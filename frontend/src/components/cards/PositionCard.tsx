    ...
    <>
      <div className="glass-card overflow-hidden group flex flex-col">
        <div className="relative aspect-video bg-[#1D1D26]">
          <img src={position.image} alt={position.title} className="w-full h-full object-cover opacity-70 group-hover:opacity-90 transition-opacity"/>
          <div className="absolute top-2 right-2"><ScorePill score={overallScore} label="Score"/></div>
        </div>
        <div className="p-4 flex flex-col flex-1">
          ...
          <p className="text-sm text-white/50 mb-3 flex-1">{position.description}</p>
          <div className="flex flex-wrap gap-1 mb-3">
            {position.keywords.map(t=><span key={t} className="text-xs px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/60">{t}</span>)}
          </div>
