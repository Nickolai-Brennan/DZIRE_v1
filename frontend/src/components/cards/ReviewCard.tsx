  ...
  return (
    <div className="glass-card overflow-hidden flex flex-col">
      <div className="relative aspect-video bg-[#1D1D26]">
        <img src={review.image} alt={review.title} className="w-full h-full object-cover opacity-70"/>
        {review.isTrophy && review.trophyLabel && (
          <div className="absolute top-2 left-2 flex items-center gap-1 bg-yellow-400/20 border border-yellow-400/40 text-yellow-400 text-xs font-bold px-2 py-1 rounded-full">
            <Trophy className="w-3 h-3"/>{review.trophyLabel}
          </div>
        )}
        <div className="absolute top-2 right-2"><ScorePill score={review.overallScore} label="Score"/></div>
      </div>
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="grid grid-cols-2 gap-1 mb-3">
          {[['Ease',review.easeOfUse],['Build',review.buildQuality],['Beginner',review.beginnerFriendly],['Value',review.valueScore]].map(([l,v])=>(
        ...
        <div className="flex gap-2">
          <Link to={`/reviews/${review.slug}`} className="flex-1 text-center py-2 bg-rose-500 hover:bg-rose-400 text-white text-sm font-semibold rounded-lg transition-colors">Read Review</Link>
