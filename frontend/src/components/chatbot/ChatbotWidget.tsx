...
const BOT_RESPONSES = [
  ...
  {
    keywords: ["advanced", "difficult", "hard", "challenge"],
    category: "Advanced",
  },
...
function getBotReply(input: string): {
  ...
  const matches = category
    ? mockPositions
        .filter((p) => p.category === category || p.difficulty <= 3)
        .slice(0, 3)
    : mockPositions.filter((p) => p.isFeatured).slice(0, 3);

  if (matches.length === 0) {
    return {
      text: "I couldn't find specific matches, but here are some popular positions:",
      suggestions: mockPositions
        .slice(0, 3)
        ...
        <div className="fixed bottom-24 right-6 z-50 w-80 bg-surface border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="bg-primary/20 px-4 py-3 border-b border-white/10">
            ...
            <p className="text-xs text-textMuted">
              Your personal position assistant
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 max-h-80">
            {messages.map((msg) => (
              ...
              >
                <div
                  className={`max-w-[85%] rounded-xl px-3 py-2 text-sm ${msg.role === "user" ? "bg-primary text-white" : "bg-surfaceAlt text-textPrimary"}`}
                >
                  <p>{msg.text}</p>
