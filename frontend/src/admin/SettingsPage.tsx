...
export const SettingsPage: React.FC = () => {
  ...
  const labelClass =
    "block text-xs text-textMuted uppercase tracking-wider mb-1";
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSiteSettings((s) => ({ ...s, siteName: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSiteSettings((s) => ({ ...s, tagline: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSiteSettings((s) => ({ ...s, siteUrl: e.target.value }))
                }
              />
            </div>
            <div className="flex items-center justify-between">
              ...
              >
                <span
                  className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${siteSettings.maintenanceMode ? "left-5" : "left-0.5"}`}
                />
              </button>
            </div>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-textMuted">
                  Enable event tracking across the site
                </p>
              </div>
              <button
                onClick={() =>
              ...
              >
                <span
                  className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${siteSettings.analyticsEnabled ? "left-5" : "left-0.5"}`}
                />
              </button>
            </div>
          </div>
        ...
        <section className="bg-surface rounded-2xl p-6 border border-white/8">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mb-5">
            <p className="text-sm text-yellow-300 font-medium">
              ⚠ Temporary admin credentials are active.
            </p>
            <p className="text-xs text-yellow-400/80 mt-1">
              Change your admin password before going to production.
            </p>
          </div>
          <div className="space-y-4">
            <div>
          ...
          <div className="space-y-4">
            {[
              ...
              },
              {
                key: "vipPurchase",
                label: "VIP Purchase",
                desc: "Notify when a new VIP subscription is purchased",
              },
              {
                key: "affiliateClick",
                label: "Affiliate Clicks",
                desc: "Log every affiliate CTA click to notifications",
              },
              {
                key: "dailyDigest",
                label: "Daily Digest Email",
                desc: "Receive a daily summary of key metrics",
              },
            ].map((item) => (
              <div key={item.key} className="flex items-center justify-between">
                ...
                <button
                  onClick={() =>
                    setNotifSettings((s) => ({
                      ...s,
                      [item.key]: !s[item.key as keyof typeof s],
                    }))
                  }
                ...
                >
                  <span
                    className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${notifSettings[item.key as keyof typeof notifSettings] ? "left-5" : "left-0.5"}`}
                  />
                </button>
              </div>
            ))}
        ...
        <section className="bg-surface rounded-2xl p-6 border border-white/8">
          ...
          <div className="space-y-3">
            {[
              {
                name: "Stripe",
                desc: "Payment processing for VIP subscriptions",
                connected: false,
              },
              {
                name: "Mailchimp / ConvertKit",
                desc: "Newsletter delivery and automation",
                connected: false,
              },
              {
                name: "Google Analytics",
                desc: "Advanced traffic analytics",
                connected: false,
              },
              {
                name: "Google Search Console",
                desc: "SEO rankings and crawl data",
                connected: false,
              },
            ].map((integration) => (
              <div
