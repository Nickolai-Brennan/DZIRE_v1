...
const keywords: KeywordRanking[] = [
  {
    keyword: "intimate positions guide",
    page: "/positions",
    position: 3,
    prevPosition: 5,
    searchVolume: 14400,
  ...
  {
    keyword: "missionary position tips",
    page: "/positions/classic-missionary",
    position: 2,
    prevPosition: 2,
    searchVolume: 8800,
    clicks: 980,
  ...
  {
    keyword: "aftercare definition",
    page: "/dictionary/aftercare",
    position: 1,
    prevPosition: 3,
    searchVolume: 3200,
    clicks: 720,
  ...
  {
    keyword: "digital intimacy essay",
    page: "/magazine",
    position: 15,
    prevPosition: 22,
    searchVolume: 2800,
  ...
  {
    keyword: "beginner sex positions",
    page: "/positions?filter=Beginner",
    position: 5,
    prevPosition: 4,
    searchVolume: 31000,
    clicks: 3200,
  ...
  {
    keyword: "couple connection tips",
    page: "/magazine",
    position: 19,
    prevPosition: 18,
    searchVolume: 9400,
  ...
  {
    keyword: "consent meaning",
    page: "/dictionary/consent",
    position: 8,
    prevPosition: 11,
    searchVolume: 12000,
    clicks: 1100,
...
const getMovement = (current: number, prev: number) => {
  ...
  if (diff > 0)
    return { icon: TrendingUp, color: "text-green-400", label: `+${diff}` };
  if (diff < 0)
    return { icon: TrendingDown, color: "text-red-400", label: `${diff}` };
...
const contentNeedingUpdate = [
  {
    page: "/magazine/modern-desire",
    keyword: "digital intimacy",
    score: 58,
    issue: "Word count too low, missing H2 tags",
  },
  {
    page: "/positions?filter=Beginner",
    keyword: "beginner sex positions",
    score: 64,
    issue: "Title tag too long, meta description missing",
  },
  {
    page: "/reviews",
    keyword: "adult product reviews",
    issue: "Rank dropped from #8 to #12 — needs fresh content",
    score: 61,
  },
  {
    page: "/magazine",
    keyword: "couple connection tips",
    issue: "Low click-through rate — improve title/meta",
    score: 55,
  },
];
    ...
    <AdminLayout title="Search Rankings">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span
                      className={`text-sm font-black ${getPositionColor(k.position)}`}
                    >
                      #{k.position}
                    </span>
                    <span
                      className={`flex items-center gap-1 text-xs font-bold ${mv.color}`}
                    >
                      <mv.icon className="w-3 h-3" />
                      {mv.label}
                    </span>
                  </div>
                </div>
              ...
              {keywords.map((k) => {
                ...
                return (
                  <tr key={k.keyword}>
                    ...
                    <td className="py-3 text-xs text-textMuted font-mono truncate max-w-[160px]">
                      {k.page}
                    </td>
                    <td className="py-3 text-center">
                      <span
                        className={`text-base font-black ${getPositionColor(k.position)}`}
                      >
                        #{k.position}
                      </span>
                    </td>
                    <td className="py-3 text-center">
                      <span
                        className={`flex items-center justify-center gap-1 text-xs font-bold ${mv.color}`}
                      >
                        <mv.icon className="w-3 h-3" />
                        {mv.label}
                      </span>
      ...
      <div className="bg-surface rounded-2xl p-6 border border-white/8 mb-8">
        ...
        <div className="space-y-4">
          {contentNeedingUpdate.map((item) => (
            <div
              key={item.page}
              className="flex items-start gap-4 bg-surfaceAlt rounded-xl p-4 border border-white/8"
            >
              ...
              <div className="text-center shrink-0">
                <p
                  className={`text-xl font-black ${item.score >= 65 ? "text-yellow-400" : "text-red-400"}`}
                >
                  {item.score}
                </p>
      ...
      <div className="bg-surface rounded-2xl p-6 border border-white/8">
        ...
        <p className="text-sm text-textMuted mb-5">
          Connect these tools to pull live ranking data automatically.
        </p>
