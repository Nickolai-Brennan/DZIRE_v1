...
const countryData = [
  { country: "United States", code: "US", visitors: 68400, pct: 43.5, conversions: 4.2 },
  { country: "United Kingdom", code: "GB", visitors: 21300, pct: 13.6, conversions: 3.8 },
  { country: "Canada", code: "CA", visitors: 14200, pct: 9.0, conversions: 3.5 },
  { country: "Australia", code: "AU", visitors: 11800, pct: 7.5, conversions: 4.0 },
  { country: "Germany", code: "DE", visitors: 8900, pct: 5.7, conversions: 2.9 },
  { country: "France", code: "FR", visitors: 6700, pct: 4.3, conversions: 2.6 },
  { country: "Netherlands", code: "NL", visitors: 4300, pct: 2.7, conversions: 3.1 },
    ...
    <AdminLayout title="Geographic Stats">
      ...
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <p className="text-sm text-blue-300">
          Geographic data is anonymized and aggregated. No individual user locations are tracked or stored.
          All data is based on IP geolocation at a country/region level only.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-surface rounded-2xl p-6 border border-white/8">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-lg font-bold text-textPrimary">Conversion Rate by Country</h2>
          </div>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={conversionByRegion}>
              ...
              />
              <Bar dataKey="rate" fill="#F5C451" radius={[6, 6, 0, 0]} name="Rate %" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Top Regions */}
        <div className="bg-surface rounded-2xl p-6 border border-white/8">
          <h2 className="text-lg font-bold text-textPrimary mb-5">Top US States / Regions</h2>
            ...
            <tbody className="divide-y divide-white/5">
              {regionData.map((row) => (
                <tr key={row.region}>
                  ...
                  <td className="py-3 text-right text-textMuted">{row.visitors.toLocaleString()}</td>
                  <td className="py-3 text-right text-green-400 font-bold">{row.conversions}%</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Country Table */}
      <div className="bg-surface rounded-2xl p-6 border border-white/8">
        <h2 className="text-lg font-bold text-textPrimary mb-5">Traffic by Country</h2>
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-textMuted text-xs uppercase tracking-wider">
          ...
          <tbody className="divide-y divide-white/5">
            {countryData.map((row) => (
              <tr key={row.country}>
                <td className="py-3 text-textPrimary font-medium">{row.country}</td>
                ...
                <td className="py-3 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <span className="text-textMuted text-xs w-10 text-right">{row.pct}%</span>
                  </div>
                </td>
                <td className="py-3 text-right text-green-400 font-bold">{row.conversions}%</td>
              </tr>
