...
const MOCK_QUEUE: QueuePost[] = [
  ...
  },
  {
    ...
    platform: "TikTok",
    status: "ready",
    scheduled_at: "2025-07-09",
  },
  {
    id: "4",
    title: "Collection Announcement",
    platform: "Facebook",
    status: "publishing",
    scheduled_at: "2025-07-09",
  },
...
export const PublishingQueuePage: React.FC = () => {
  const countByStatus = (s: QueueStatus) =>
    MOCK_QUEUE.filter((p) => p.status === s).length;
    ...
    <AdminLayout title="Publishing Queue">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
        {STATUSES.map((s) => (
          <div
            key={s}
            className="p-4 bg-surfaceAlt rounded-xl border border-white/10 text-center"
          >
            <p className={`text-2xl font-bold ${STATUS_HEADER_COLORS[s]}`}>
              {countByStatus(s)}
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {STATUSES.map((s) => {
          ...
          return (
            <div
              key={s}
              className="bg-surfaceAlt rounded-xl border border-white/10 overflow-hidden"
            >
              <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
                <span
                  className={`text-sm font-medium capitalize ${STATUS_HEADER_COLORS[s]}`}
                >
                  {s}
                </span>
                <span className="text-xs text-textMuted bg-white/5 px-2 py-0.5 rounded-full">
                  {posts.length}
                </span>
              </div>
              <div className="p-3 space-y-2">
                ...
                {posts.map((post) => (
                  <div
                    key={post.id}
                    className="p-3 bg-background rounded-lg border border-white/5"
                  >
